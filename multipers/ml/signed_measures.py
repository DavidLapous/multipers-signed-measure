...
class SimplexTree2SignedMeasure(BaseEstimator,TransformerMixin):
	...
	def __init__(self, 
		...
		self.resolution = np.inf if infer_filtration_strategy == 'exact' and resolution is None else 100
		self.plot=plot
		self.sparse=sparse
		...
		self.expand = expand
		self._refit_grid = None # will only refit the grid if filtration_grid has never been given.
		self.seed=seed
		...
		assert resolution is not None or filtration_grid is not None or infer_filtration_strategy == "exact" or self.individual_grid, 'For non exact filtrations, a resolution has to be specified.'
	...
	def _infer_filtration(self,X):
		...
		filtration_grid = reduce_grid(filtrations_values, resolutions = self.resolution, strategy = self.infer_filtration_strategy) # TODO :use more parameters
		self.filtration_grid=filtration_grid
	...
	def fit(self, X, y=None): # Todo : infer filtration grid ? quantiles ?
		...
		self.individual_grid = self.individual_grid if self.individual_grid is not None else self.infer_filtration_strategy in ["regular_closest", "exact", "quantile", "partition"]
	...
	def transform1(self, simplextree, filtration_grid=None, _reconversion_grid=None):
		...
		if self.individual_grid:
			filtration_grid = st.get_filtration_grid(grid_strategy=self.infer_filtration_strategy, resolution=self.resolution)
		...
		if self._möbius_inversion:
			if self.expand and len(int_degrees) > 0:	st.expansion(np.max(int_degrees)+1)
			...
			if self.expand and len(self.rank_degrees) > 0 :	st.expansion(np.max(self.rank_degrees)+1)
			...
			if self.expand and len(self.rank_degrees) > 0 :	st.expansion(np.max(self.rank_degrees)+1)
			signed_measures_rank = mp.rank_invariant(
					simplextree=st,degrees=self.rank_degrees,
					plot=self.plot,
			) if len(self.rank_degrees) >0 else []
			...
			signed_measures_euler = [euler_surface(
					simplextree=st,
					plot=self.plot,
			)[1]] if None in self.degrees else []
		...
		if self._möbius_inversion and not self.sparse:
...
class SimplexTrees2SignedMeasures(SimplexTree2SignedMeasure):
	...
	def transform(self, X):
		...
		return Parallel(n_jobs=self.n_jobs, prefer="threads")(delayed(todo)(x) for x in tqdm(X, disable=not self.progress, desc="Computing Signed Measures from simplextrees."))
...
class SignedMeasureFormatter(BaseEstimator,TransformerMixin):
	...
	def _get_filtration_bounds(self, X, axis):
		stuff = [np.concatenate([sm[axis][d][0] for sm in X], axis=0) for d in range(len(X[axis][0]))]
	...
	def fit(self, X, y=None):
		...
		# if (self.unsparse or self.integrate) and self._is_input_sparse or self.normalize:
		...
		if self._is_input_sparse and self.normalize:
			...
			## ax, degree, filtration_values
			# filtration_values = [[np.concatenate([x[ax][degree][0] for x in X]) for degree in range(self._num_degrees)] for ax in axis]
			
			## np.unique for each ax, degree, parameter
			# filtration_values = [[reduce_grid(f_ax_d.T, resolutions=self.resolution, strategy=self.infer_grid_strategy) for f_ax_d in f_ax] for f_ax in filtration_values]

			## share with degree (rips has no 0 valued edges)
			filtration_values = [np.concatenate([x[ax][degree][0] for x in X for degree in range(self._num_degrees)]) for ax in axis]
			
			filtration_values = [reduce_grid(f_ax.T, resolutions=self.resolution, strategy=self.infer_grid_strategy) for f_ax in filtration_values]
			self._infered_grids = [[np.concatenate([[f[0]-1],f, [f[-1]+1]], axis=0) for f in f_ax] for f_ax in filtration_values]
	...
	def transform(self,X):
		...
		if self._normalization_factors is not None:
			if self.n_jobs >1:
				out = Parallel(n_jobs=self.n_jobs, prefer="threads")(delayed(todo_rescale)(x) for x in out)
			else:
		...
		if self._is_input_sparse:
			if self.integrate:
				...
				# if self.axis != -1:
				ax = slice(None) if self.axis is None else self.axis
				out = np.asarray([[self._integrate_measure(x[degree],filtrations=filtrations[ax][degree]) for degree in range(self._num_degrees)] for x in out])
				if self.flatten:
...
class SignedMeasure2Convolution(BaseEstimator,TransformerMixin):
	...
	def __init__(self, 
		  ...
		  infer_grid_strategy:str="partition",
		  progress:bool=False, 
		  backend:str='pykeops',
		  plot:bool=False,
		  **kwargs):
	...
	def fit(self, X, y=None):
		...
		if self._refit:
			...
			self.filtration_grid = reduce_grid(filtrations_values=pts, strategy=self.infer_grid_strategy, resolutions=self.resolution)
			# print('Done.')
	...
	def _plot_imgs(self, imgs:Iterable[np.ndarray], size=4):
		...
		if num_degrees==1: axes=[axes]
		if len(imgs) == 1: axes = [axes]
		axes = np.asarray(axes)
		for i, img in enumerate(imgs):
			for j, img_of_degree in enumerate(img):
				plot_surface(self.filtration_grid, img_of_degree, ax=axes[j,i], cmap='Spectral')
	def transform(self,X):
		...
		if self._is_input_sparse:
			...
			out =  Parallel(n_jobs=self.n_jobs, prefer="threads")(delayed(todo)(self, signed_measures) for signed_measures in tqdm(X, desc="Computing images", disable = not self.progress))
