...
class KDE():
	...
	def fit(self, X, sample_weights=None, y=None):
		...
		match self.kernel:
			...
			case _:
				assert callable(self.kernel), "Custom kernel has to be callable, (x:LazyTensor(n,1,D),y:LazyTensor(1,m,D),bandwidth:float) ---> kernel matrix"
	...
	def exponential_kernel(x_i,y_j, bandwidth):
		exponent =  - ((x_i - y_j).abs()/bandwidth).sum(dim=2)
		kernel = (exponent).exp()/bandwidth
		return kernel
	@staticmethod
