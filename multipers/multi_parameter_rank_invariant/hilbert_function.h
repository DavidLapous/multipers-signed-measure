		...
		for (const auto& barcode : barcodes){ // TODO range view cartesian product
			...
			for(const auto &bar : barcode){
				...
				if (!mobius_inverion){
					...
					for (index_type b = birth; b < std::min(border, d); b++){
...
void _rec_get_hilbert_surface(
	...
	tbb::parallel_for(0, grid_shape[coordinate_to_iterate+1], [&](index_type z){
		...
		_rec_get_hilbert_surface(st_multi, thread_simplex_tree,out,grid_shape, degrees,coordinates_to_compute, _fixed_values, mobius_inverion,zero_pad);
	});
...
void get_hilbert_surface(
	...
	_rec_get_hilbert_surface(st_multi, thread_simplex_tree,out,grid_shape, degrees,coordinates_to_compute, fixed_values, mobius_inverion,zero_pad);
...
std::pair<std::vector<std::vector<indices_type>>, std::vector<dtype>> get_hilbert_signed_measure(
	...
	if (zero_pad){
		...
		for (auto i=1u; i< st_multi.get_number_of_parameters()+1; i++) grid_shape[i]--; // get hilbert surface computes according to grid_shape.
	...
	arena.execute([&]{
		get_hilbert_surface(st_multi,container, grid_shape, degrees, coordinates_to_compute, fixed_values, true, zero_pad);
	});
...
void get_hilbert_surface_python(
	...
	tensor::static_tensor_view<dtype, indices_type> container(data_ptr,grid_shape); // assumes its a zero tensor
	std::vector<indices_type> coordinates_to_compute(st_multi.get_number_of_parameters());
	for (auto i=0u; i< st_multi.get_number_of_parameters()+1; i++) coordinates_to_compute[i] = i;
	// for (auto [c,i] : std::views::zip(coordinates_to_compute, std::views::iota(0,st_multi.get_number_of_parameters()))) c=i;
	std::vector<indices_type> fixed_values(st_multi.get_number_of_parameters());
	oneapi::tbb::task_arena arena(n_jobs); // limits the number of threads
	arena.execute([&]{
		get_hilbert_surface(st_multi,container, grid_shape, degrees, coordinates_to_compute, fixed_values, mobius_inversion, zero_pad);
	});
	if (mobius_inversion)
		for (indices_type axis=2u; axis< st_multi.get_number_of_parameters()+1; axis++)
			container.differentiate(axis);
