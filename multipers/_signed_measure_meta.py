...
def signed_measure(
	...
	**infer_grid_kwargs
	...
	if degrees == [None] and degree is not None:
		degrees = [degree]
	...
	if invariant in ["rank_invariant", "rank"]:
		...
		sms = smri(simplextree_, mass_default=mass_default, degrees=degrees, plot=plot)
	elif degrees == [None]:
		...
		sms = hilbert_signed_measure(simplextree_,degrees=degrees, mass_default=mass_default, verbose=verbose,plot=plot, n_jobs=n_jobs)
