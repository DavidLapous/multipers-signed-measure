...
cdef extern from "multi_parameter_rank_invariant/rank_invariant.h" namespace "Gudhi::multiparameter::rank_invariant":
	void compute_rank_invariant_python(const intptr_t, tensor_dtype* , const vector[indices_type], const vector[indices_type], indices_type) except + nogil
	

def rank_invariant(simplextree, vector[indices_type] degrees, mass_default=None, plot=False, indices_type n_jobs=0, bool verbose=False):
	"""
	...
	with nogil:
		compute_rank_invariant_python(simplextree_ptr, container_ptr,c_grid_shape,degrees, n_jobs)
...
def signed_measure(simplextree, vector[indices_type] degrees, mass_default=None, plot=False, indices_type n_jobs=0, bool verbose=False):
	"""
	Computes the signed measures given by the decomposition of the hilbert function.
