...
def test_1():
	...
	assert (mma_pymodule[0].get_birth_list(), mma_pymodule[0].get_death_list())==(
		([[0.0, 1.0], [1.0, 0.0]], [[np.inf, np.inf]])
	)
...
def test_img():
	...
	mod = simplextree.persistence_approximation(box = [[0,0,0,0],[5,5,5,5]])
...
def test_img2():
	st = mp.SimplexTreeMulti(num_parameters=2)
	st.insert([0], [1,2])
	mod = st.persistence_approximation(box=[[0,0],[4,4]])
	assert np.isclose(
